...
from document_manager import ESConnector
...
class Document(BaseModel):
    """ Typing very simply the document to string"""
    text: str
...
async def create_document(document: Document):
    ...
    try:
        document_id = es.store_document(document.text)
...
async def get_document(document_id: str):
    ...
    try:
        document = es.retrieve_document(document_id)
...
@app.get("/search")
async def search_documents(q: str = Query(None, min_length=3, max_length=100),
                           summarize: bool = False):
    logging.info(f"Search endpoint called with query: {q}, summarize: {summarize}")
    try:
        search_results = es.search(index="documents",
                                   body={"query": {"match": {"text": q}}})
        hits = search_results['hits']['hits']
        results = [{"document_id": hit["_id"],
                    "text_snippet": hit["_source"]["text"][:100]} for hit in hits]
        ...
        if summarize:
            ...
            if summarized_text == "An error occurred while summarizing the documents.":
                ...
                return {"results": results,
                        "warning": "Failed to summarize documents, returning original search results."}
            ...
            return {"summarized_text": summarized_text,
                    "reranked_results": reranked_summarized_results}
        else:
